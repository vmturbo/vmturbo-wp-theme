...
<div class="container">
	...
	<div class="col-md-4">
		...
		<?php } ?>
				...
				<?php } elseif( get_field('product_download') == true) { ?>
					<form id="mktoForm_<?php if(get_field('license_type') == "community") { ?>503<?php } elseif(get_field('license_type') == "trial") { ?>502<?php } elseif(get_field('license_type') == "freemium") { ?>1015<?php } elseif(get_field('license_type') == "freemiumTrial") { ?>1014<?php } elseif(get_field('license_type') == "saas") { ?>1131<?php } else { the_field('marketo_form_code'); } ?>"></form>
				<?php } else { ?>
		...
		<?php if(get_field('product_download') && !is_page('28585')) { ?>
			...
			jQuery(document).ready(function($){
				...
				MktoForms2.loadForm("//app-sjp.marketo.com", "303-ZIH-630", <?php if(get_field('license_type') == "community") { ?>503<?php } elseif(get_field('license_type') == "trial") { ?>502<?php } elseif(get_field('license_type') == "freemium") { ?>1015<?php } elseif(get_field('license_type') == "freemiumTrial") { ?>1014<?php } else { the_field('marketo_form_code'); } ?>, function(form){
				...
				<?php } ?>
					...
					form.onSubmit(function(){
						...
						<?php if(get_field('license_type') == "community") { ?>
							var ty = tH("community");
						<?php } elseif(get_field('license_type') == "trial") { ?>
							var ty = tH("trial");
						<?php } elseif(get_field('license_type') == "freemium") { ?>
							var ty = tH("freemium");
						<?php } elseif(get_field('license_type') == "freemiumTrial") { ?>var ty = tH("freemiumTrial");
						<?php } else { ?>
							var ty = tH("trial");
						<?php }  ?>
		...
		<script type="text/javascript">wistiaEmbed = document.getElementById("wistiaframe").wistiaApi;wistiaEmbed.bind("end", function() {jQuery( "#fademe" ).fadeOut( "slow", function() {});});wistiaEmbed.bind("secondchange", function (s) {if(s === 180) {jQuery('.mktoButtonRow .alert').hide();jQuery('.mktoButtonRow .btn-vmt').fadeIn();}});jQuery(document).ready(function(){jQuery('.mktoButtonRow .btn-vmt').css('display','none');jQuery('.mktoButtonRow span').append('<div class="alert">You must watch the video before submitting this form</div>');})</script>
		</div>
	</div>
</div>
</div>